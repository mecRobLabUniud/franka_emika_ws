...
import sys
sys.path.append('/home/panda/Desktop/demo/')
import urdf2casadi.urdfparser as u2c
import time
from write_read_plot_module import reader
...
sol = None
traj_duration_prec = ubx

def cost(x, traj_duration_prec):

    return x*p1 + ca.fabs(x - traj_duration_prec)*p2
...
if __name__ == "__main__":
    ...
    t, q, q_p, q_pp, q_ppp, tau, tau_p = reader(path_name, 'q', 'hyb')

    sol = create_solver()

    p = ca.DM([q, q_p, q_pp, [traj_duration_prec, traj_duration_prec, traj_duration_prec, traj_duration_prec, traj_duration_prec, traj_duration_prec, traj_duration_prec]])
    r = sol(x0 = traj_duration_prec, p = p, lbx = lbx, ubx = ubx, ubg = 0)
    
    stop_duration = r['x'].__float__()
    
    traj_duration_prec = stop_duration
