...
def CallbackFollowJointTrajectoryResult(data):
    global status
...
def ExecuteTask(q_traj, traj_durations):
    global outfile_q, t_i
    ...
    sub_follow_joint_trajectory_result = rospy.Subscriber('/position_joint_trajectory_controller/follow_joint_trajectory/result', FollowJointTrajectoryActionResult, CallbackFollowJointTrajectoryResult)
    ...
    outfile_q = open("/home/panda/Documents/Data_Collision_Avoidance/q_robot_stop.xml", 'w')
    ...
    for i in range(len(traj_durations) - 1):
        ...
        q_traj_pair.append(deepcopy(q_traj[i + 1]))
        ExecuteTrajectory(deepcopy(q_traj_pair), traj_durations[i + 1], control_publisher)
...
if __name__ == '__main__':
    ...
    file = open("/home/panda/Desktop/demo/DEMO_COMP/p_traj.txt", 'r')
    ...
    file = open("/home/panda/Desktop/demo/DEMO_COMP/t_traj.txt", 'r')
    ...
    # Write trajectory message
    point = JointTrajectoryPoint()
    point.positions = deepcopy(q_traj[0])
    point.velocities = [0, 0, 0, 0, 0, 0, 0]
    point.accelerations = [0, 0, 0, 0, 0, 0, 0]
    point.effort = []
    ...
    print("This will start the admittance control task")
    input("Press Enter to continue...")

    ExecuteTask(deepcopy(np.array(q_traj)), deepcopy(np.array(t_traj)))
