...
from write_read_plot import writer
import os
...
def TrajPoly5(qi, qi_p, qi_pp, qf, qf_p, qf_pp, traj_duration, time_step):
    ...
    traj = np.vstack((q, q_p, q_pp, t))
    
    return traj

def CalcValidTrajServer(q_start, q_end, traj_duration, traj_step):

    global t, q, q_p, q_pp, q_ppp, tau, tau_p
    ...
    for joint_num in range(7):
        ...
        t[:] = deepcopy(np.transpose(traj[3, :]))
...
if __name__ == "__main__":
    ...
    q_start = [-0.5, 0.05, -0.1, -2.0, 0.0, 1.571, 0.785]
    q_end = [0.0, -0.785, 0.0, -2.356, 0.0, 1.571, 0.785]
    traj_duration = 4
    traj_step = 1/rate

    CalcValidTrajServer(q_start, q_end, traj_duration, traj_step)
    ...
    path_name = '/home/panda/Desktop/trajpoly5'

    if not os.path.exists(path_name):
        os.makedirs(path_name)

    writer(path_name, t, q, q_p, q_pp, q_ppp, tau, tau_p, 'q', '_test')